  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-12 md:py-16">
      <div className="mx-auto max-w-6xl">
        {/* Header */}
        <div className="mb-12">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#22C55E] mb-3">News & Updates</p>
            ...
            <span
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium border cursor-default ${
                cat === "All"
                  ...
                  : "border-white/[0.08] bg-[#0F1729] text-[#94A3B8]"
              }`}
            >
              {cat}
            </span>
        ...
        {featured.length > 0 && (
          <div className="mb-10">
            <h2 className="text-xs font-semibold uppercase tracking-widest text-[#64748B] mb-5">Featured</h2>
        ...
        {/* All posts */}
        <div>
          <h2 className="text-xs font-semibold uppercase tracking-widest text-[#64748B] mb-5">Latest</h2>
        ...
        <div className="mt-16 rounded-2xl border border-white/[0.08] bg-[#0F1729] p-8 text-center">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#22C55E] mb-3">About this publication</p>
          <p className="text-[#94A3B8] text-sm max-w-xl mx-auto leading-relaxed">
            ...
            and international founder journeys. All articles are written or reviewed by Guillaume Gruyters, a
...
function PostCard({ post, featured = false }: { post: Post; featured?: boolean }) {
  return (
    <Link href={`/news/${post.slug}`} className="group block">
      <article
        className={`card-hover rounded-2xl border border-white/[0.08] bg-[#0F1729] p-6 h-full ${
          featured ? "md:p-8" : ""
        }`}
      >
        ...
        <h3
          className={`font-bold text-[#F8FAFC] group-hover:text-[#22C55E] transition-colors mb-2 leading-snug ${
            featured ? "text-xl md:text-2xl" : "text-lg"
          }`}
        >
          {post.title}
        </h3>
        <p className="text-[#94A3B8] text-sm leading-relaxed mb-4">{post.excerpt}</p>
